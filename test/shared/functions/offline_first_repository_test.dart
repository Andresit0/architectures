...
void main() {
  group('fetchOrFallback', () {
    ...
    test('falls back to local when remote fails with NetworkError',
        () async {
      final result = await fetchOrFallback(
    ...
    test('falls back to local when remote fails with ServerUnreachableError',
        () async {
      final result = await fetchOrFallback(
        remote: () async => Failure<String>(
    ...
    test('does NOT fall back when remote fails with UnexpectedError',
        () async {
      final result = await fetchOrFallback(
        remote: () async => Failure<String>(
