...
void main() {
  group('Architecture Dependency Rules', () {
    test(
      ...
      () {
        final files = Directory('lib/features/auth/domain/entities')
            ...
            .where(
              (f) =>
                  f.path.endsWith('.dart') && !f.path.contains('.freezed.dart'),
            );
    ...
    test('Rule 3: No .g.dart files in domain/entities/ or shared/models/', () {
      ...
      expect(domainG, isEmpty, reason: 'domain/entities/ contiene .g.dart');
    ...
    test('Rule 5: features/ NO importan de otros features/', () {
      final featureDirs = Directory(
        'lib/features',
      ).listSync().whereType<Directory>().map((d) => d.path.split('/').last);
